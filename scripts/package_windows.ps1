...
if (Test-Path $isccPath) {
    ...
    Write-Host "[OK] Setup installer created: dist\DontForget_Setup_v1.2.3.exe" -ForegroundColor Green
} else {
...
$zipOutput = "$PSScriptRoot\..\dist\DontForget_Portable_v1.2.3.zip"
if (Test-Path $zipOutput) { Remove-Item $zipOutput -Force }
Compress-Archive -Path "$releaseDir\*" -DestinationPath $zipOutput -Force
Write-Host "[OK] Portable zip created: dist\DontForget_Portable_v1.2.3.zip" -ForegroundColor Green
